...
# Try to import action recorder
try:
    from computer_use_agent.action_recorder import record_tool_action, get_recorder
    ACTION_RECORDER_AVAILABLE = True
    print("✅ Action recorder imported successfully")
except ImportError as e:
    print(f"❌ Failed to import action recorder: {e}")
    ACTION_RECORDER_AVAILABLE = False
    def record_tool_action(*args, **kwargs):
        pass
    def get_recorder():
        return None
...
# Inter-key delay in milliseconds for human-like typing.
# Can be overridden with environment variable TYPING_DELAY_MS.
TYPING_DELAY_MS = int(os.getenv("TYPING_DELAY_MS", "100"))
# Number of characters per xdotool type invocation.
# Can be overridden with environment variable TYPING_GROUP_SIZE.
TYPING_GROUP_SIZE = int(os.getenv("TYPING_GROUP_SIZE", "1"))
        ...
        if action in ("key", "type"):
            ...
            if action == "key":
                ...
                for chunk in chunks(text, TYPING_GROUP_SIZE):
                    ...
                    # Ensure a perceptible delay between chunks for more human-like typing
                    await asyncio.sleep(max(TYPING_DELAY_MS, 1) / 1000)
...
class ComputerTool20250124(BaseComputerTool, BaseAnthropicTool):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        try:
            if ACTION_RECORDER_AVAILABLE:
                print(f"🔧 Recording tool action: {action}")
                # Use the existing recorder instance
                recorder = get_recorder()
                if recorder and recorder.start_time is not None:
                    record_tool_action(
                        tool_name="computer",
                        action=action,
                    ...
                    print(f"⚠️  No active tool recorder session, skipping: {action}")
            else:
                print(f"⚠️  Action recorder not available, skipping: {action}")
    ...
    async def _execute_action(
        ...
        **kwargs,
        ...
        """Execute the actual action (moved from __call__)."""
        ...
        if action in (
            ...
            if text is not None:
                raise ToolError(f"text is not accepted for {action}")
            # If a coordinate is provided, perform a smooth segmented move before clicking
            if coordinate is not None:
                dest_x, dest_y = self.validate_and_get_coordinates(coordinate)

                # Current mouse position
                result_pos = await self.shell(
                    f"{self.xdotool} getmouselocation --shell", take_screenshot=False
                )
                output = result_pos.output or ""
                # Parse strictly without try/except; let errors surface for clarity
                cur_x = int(output.split("X=")[1].split("\n")[0])
                cur_y = int(output.split("Y=")[1].split("\n")[0])

                # Segmented path parameters
                dx = dest_x - cur_x
                dy = dest_y - cur_y
                total_seconds = (
                    float(duration)
                ...
                step_dt = 0.015
                min_steps = 10
                steps = min(60, max(min_steps, int(total_seconds / step_dt)))
                ...
                for i in range(1, steps):
                    ...
                    yi = round(cur_y + dy * t)
                    await self.shell(
                        f"{self.xdotool} mousemove {xi} {yi}", take_screenshot=False
                    )
                    await asyncio.sleep(step_dt)

                # Final position + click
                pre_click_cmds = [self.xdotool, f"mousemove --sync {dest_x} {dest_y}"]
                if key:
                    pre_click_cmds.append(f"keydown {key}")
                await self.shell(" ".join(pre_click_cmds), take_screenshot=False)

                click_cmds = [self.xdotool, f"click {CLICK_BUTTONS[action]}"]
                if key:
                    click_cmds.append(f"keyup {key}")
                return await self.shell(" ".join(click_cmds))

            # No coordinate provided: just click at current position (original behavior)
